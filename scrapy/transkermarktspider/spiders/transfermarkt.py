...
class TransfermarktSpider(scrapy.Spider):
        ...
        for championship in championships:
            for i in range(2010, 2022):
                if (championship == "serie-a"):
                    link = "https://www.transfermarkt.com/" + championship + "/startseite/wettbewerb/IT1/saison_id/" + str(
                        i)
                elif (championship == "premier-league"):
                    link = "https://www.transfermarkt.com/" + championship + "/startseite/wettbewerb/GB1/saison_id/" + str(
                        i)
                elif (championship == "primera-division"):
                    link = "https://www.transfermarkt.com/" + championship + "/startseite/wettbewerb/ES1/saison_id/" + str(
                        i)
                elif (championship == "ligue-1"):
                    link = "https://www.transfermarkt.com/" + championship + "/startseite/wettbewerb/FR1/saison_id/" + str(
                        i)
                elif (championship == "bundesliga"):
                    link = "https://www.transfermarkt.com/" + championship + "/startseite/wettbewerb/L1/saison_id/" + str(
                        i)
                request = response.follow(link, callback=self.parse_squad_in_a_championship, meta={'year': str(i)})
                yield request
        ...
        for squad in squads:
            ...
            if(name_squad):
                ...
                request_player = response.follow(link,
                                                 callback=self.parse_player_in_a_team,meta={'squad_name':name_squad, 'year': response.meta['year'] })
                yield request_player
    ...
    def parse_player_in_a_team(self,response):
        ...
        for player in players:
            ...
            money = self.change_money(value_player)
            ...
            create_link="https://www.transfermarkt.com/"+name_link+"/leistungsdatendetails/spieler/"+id_link+"/plus/0?saison="+response.meta['year']+"&verein=&liga=&wettbewerb=&pos=&trainer_id="
            """if (name_player):
                yield {
            ...
            if(name_player):
                request_player = response.follow(create_link,
                                                 callback=self.parse_bio_player,
                                                 meta={'player_name':name_player, 'age':birthday, 'role':role, 'value_player':money, 'squad_name': response.meta['squad_name'], 'year': response.meta['year']})
                yield request_player
